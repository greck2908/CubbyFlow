    ...
    flatbuffers::Offset<fbs::ParticleSystemData3>* fbsParticleSystemData) const
{
    ...
    std::vector<flatbuffers::Offset<fbs::MatrixParticleData3>> matrixDataList;
    for (const auto& matrixData : m_matrixDataList)
    {
        std::vector<fbs::Matrix3x3D> newMatrixData;
        for (const auto& m : matrixData)
        {
            newMatrixData.push_back(CubbyFlowToFlatbuffers(m));
        }

        flatbuffers::Offset<fbs::MatrixParticleData3> fbsMatrixData =
            CreateMatrixParticleData3(
                *builder, builder->CreateVectorOfStructs(newMatrixData.data(),
                                                         newMatrixData.size()));
        matrixDataList.push_back(fbsMatrixData);
    }
    const flatbuffers::Offset<
        flatbuffers::Vector<flatbuffers::Offset<fbs::MatrixParticleData3>>>
    ...
    const fbs::ParticleSystemData3* fbsParticleSystemData)
{
    ...
    const flatbuffers::Vector<flatbuffers::Offset<fbs::MatrixParticleData3>>*
        fbsMatrixDataList = fbsParticleSystemData->matrixDataList();
    for (const auto fbsMatrixData : (*fbsMatrixDataList))
    {
        const flatbuffers::Vector<const fbs::Matrix3x3D*>* data =
            fbsMatrixData->data();

        m_matrixDataList.emplace_back(data->size());

        Array<Matrix<double, 3, 3>, 1>& newData = *(m_matrixDataList.rbegin());
