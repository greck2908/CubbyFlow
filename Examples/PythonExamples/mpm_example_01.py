...
def main():
    # Create MPM solver
    resX = 32
    solver = MPMSolver2(resolution=(resX, resX), domainSizeX=1.0)

    # Setup emitter
    sphere = Sphere2(center=(0.5, 1.0), radius=0.15)
    emitter = VolumeParticleEmitter2(
        implicitSurface=sphere, spacing=1.0 / (2 * resX))
    solver.particleEmitter = emitter

    # Setup collider
    anotherSphere = Sphere2(center=(0.5, 0.5), radius=0.15)
    collider = RigidBodyCollider2(surface=anotherSphere)
