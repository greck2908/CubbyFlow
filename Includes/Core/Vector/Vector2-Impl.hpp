...
#ifndef CUBBYFLOW_VECTOR2_IMPL_HPP
#define CUBBYFLOW_VECTOR2_IMPL_HPP
...
template <typename T>
Vector<T, 2>& Vector<T, 2>::operator=(Vector&& v) noexcept
{
    Set(v);
    return (*this);
}
...
template <typename T>
Vector<T, 2> Sqrt(const Vector<T, 2>& a)
{
    return Vector<T, 2>{ std::sqrt(a.x), std::sqrt(a.y) };
}
